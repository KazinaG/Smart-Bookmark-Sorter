...
- フロー詳細と例外条件の正本は `docs/dev-practices/process.md` とし、AGENTS.md には着手前ゲートのみを置く。
- 回答では確認した一次情報を明記する。
...
- `実装OK` を提示する前に、`Scope` `Done` `Non-goals` `Constraints` `対象ファイル` `検証方法` の6点を明示する。
- タスク種別ごとに必読文書を確認し、通常応答の `【確認ドキュメント】` に列挙する。
- 必読文書を未確認のまま進める場合は、`【確認ドキュメント】` に `N/A理由: ...` を明示する。
- 必読文書の最小セットは次のとおり。
  - 方針相談・仕様変更あり: `docs/policies/approval.md`, `docs/dev-practices/process.md`, `docs/policies/documentation.md`
  - 実装・検証あり: `docs/policies/approval.md`, `docs/dev-practices/process.md`, `docs/dev-practices/testing.md`, `docs/dev-practices/commit.md`
  - 反映操作（merge/push）: `docs/policies/approval.md`, `docs/dev-practices/operations.md`, `docs/dev-practices/commit.md`
...
- 応答は日本語で、通常応答本文は `## 事実` `## 推測` `## 提案` の見出しで分ける。
...
罫線はバレットリストから 1 行空けて記載し、毎回同じ長さの固定表記にします。
